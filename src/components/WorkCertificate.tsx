...
export default function WorkCertificate({ worker, employer, onClose }: Props) {
    ...
    const handlePrint = usePrint(`Certificat_Travail_${worker.nom}_${worker.prenom}`);
    ...
    return (
        <div className="flex flex-col h-full bg-gray-100 p-4 overflow-auto print:bg-white print:p-0">
            ...
            <div className="max-w-[210mm] mx-auto mb-2 text-xs text-blue-600 bg-blue-50 px-3 py-1 rounded-full border border-blue-100 flex items-center gap-2 animate-pulse print:hidden w-fit">
                ...
                <span>Ce certificat est éditable directement à l'écran avant impression</span>
            </div>
            ...
            >
                ...
                <div className="mb-8 space-y-6 text-justify leading-normal">
                    ...
                    <p contentEditable suppressContentEditableWarning>
                        A été employé(e) au sein de notre société du <strong>{formatDate(worker?.date_embauche)}</strong> au <strong>{formatDate(worker?.date_debauche)}</strong>.
                    </p>
                    ...
                    <div className="pl-4 border-l-4 border-gray-200 print:border-l-2 print:border-black" contentEditable suppressContentEditableWarning>
                        <p className="mb-2 font-semibold italic text-sm">Postes occupés :</p>
                        {worker?.position_history && worker.position_history.length > 0 ? (
                                ...
                                {worker.position_history
                                    ...
                                    .map((hist: any, idx: number) => (
                                        <li key={idx}>
                                            ...
                                            <span className="text-gray-600">Du {formatDate(hist.start_date)} au {hist.end_date ? formatDate(hist.end_date) : formatDate(worker?.date_debauche)}</span>
                                        </li>
                                    ))}
                            </ul>
                        ) : (
                            ...
                            <ul className="list-disc pl-5 text-sm">
                                <li>
                                    <span className="font-bold">{worker?.poste || "Employé(e)"}</span> {worker?.categorie_prof ? `(${worker.categorie_prof})` : ''} <br />
                                    <span className="text-gray-600">Période complète</span>
                                </li>
                            </ul>
                        )}
                    </div>
                    ...
                    <p contentEditable suppressContentEditableWarning>
                        M./Mme {worker?.nom} quitte la société libre de tout engagement et peut faire valoir ce certificat pour servir et valoir ce que de droit.
                    </p>
                </div>
